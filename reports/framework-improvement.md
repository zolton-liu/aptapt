...
- 当前开发版为 **v6.2**：可执行输入/领域契约、栈帧准确定位、版本化失败记忆、四阶段求解协议和单组件消融开关。**139 项测试通过**，尚未产生新版模型评测成绩。见 [v6.2 修改与验证](framework-v6.2-contract-pipeline.md)。下面的 2/10 是 v6.1 历史成绩，不是 v6.2 成绩。

- v5.6 原始及整理后的 87 题全量开发基线均为 11/87（12.64%）；整理版包含 7 条替换记录且通过数未增加，原始版有 7 条 SIGTERM。完整口径见 [全量审计](workflow-v5.6-all87-audit.md)。
- v6.1 在上下文与记忆改造基础上补充统一截止时间、真实证据重放、语法编辑保护、失败执行门槛与停滞结束；全套测试 **121 passed**。
- 固定同样 10 题的真实模型结果：v5.6 **4/10** → v6 **1/10** → v6.1 **2/10**。恢复一个通过项、外层超时减少，但没有证明整体优于 v5.6。详见 [最终效果报告](recovery-v6.1-random10-results.md)。
